...
field_names = ['implementation', 'file', 'length', 'running_time']
...
#     end = time()
#     writer.writerow({'implementation':"Read by char", 'file':filename, 'length': str(result), 'running_time': str(end - start)})
...
for filename in os.listdir(csv_folder):
    ...
    writer.writerow({'implementation':"Read by line", 'file':filename, 'length': str(result), 'running_time': str(end - start)})
...
for filename in os.listdir(csv_folder):
    ...
    for buffer_size in [2 ** x for x in range(12)]:
        ...
        writer.writerow({'implementation':"Read by buffer", 'file':filename, 'length': str(result), 'running_time': str(end - start)})
