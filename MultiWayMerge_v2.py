...
def extsort_Line_Line(f, k, M, d):
    ...
    while len(sortedFilesQueue) > 1:
        ...
        while count > 0:
            ...
            minRow = takeMinLine(linesList, outputFile)
            bufferListIndex = int(minRow[0])

            # If already empty
            if bufferList[bufferListIndex]['buffer'] == []:
                bufferList[bufferListIndex] = loadBuffer(bufferList[bufferListIndex], filesToSort[bufferListIndex][1], M, count)
...
def takeMinLine(linesList, outputFile):
    ...
    minRow = natsort.index_natsorted(linesList)
    minLine = natsort.natsorted(linesList)
    minBytes = bytes(minLine[0][1], 'utf-8')
    print(minBytes)
    writeln_line(outputFile, minBytes)
    # linesList = np.delete(linesList, minRow[0], axis=0)
